...
class Conv(nn.Module):
    ...
    default_act = nn.Hardswish()  # default activation (DPU-friendly; was nn.SiLU())
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.act = nn.Hardswish()  # DPU-friendly (was nn.SiLU())
...
class ECA(nn.Module):
    """SE-style channel attention — DPU-friendly drop-in replacement for the
    original ECA (which used nn.Conv1d + squeeze/transpose/Sigmoid, none of
    which map cleanly onto the Vitis-AI DPU and force a CPU subgraph split).

    Uses only AdaptiveAvgPool2d + 1x1 Conv2d + ReLU + Hardsigmoid, so the whole
    attention block stays on the DPU. Keeps the class name `ECA` and the
    `k_size` arg so MSFE(...) and any other call sites work unchanged.
    """

    def __init__(self, channel, k_size=5, r=8):
        super(ECA, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        mid = max(channel // r, 8)
        self.fc1 = nn.Conv2d(channel, mid, 1, bias=False)
        self.act = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(mid, channel, 1, bias=False)
        self.gate = nn.Hardsigmoid()

    def forward(self, x):
        # x: input features with shape [b, c, h, w]
        y = self.avg_pool(x)                 # [b, c, 1, 1]
        y = self.gate(self.fc2(self.act(self.fc1(y))))
        return x * y                         # broadcast over h, w
